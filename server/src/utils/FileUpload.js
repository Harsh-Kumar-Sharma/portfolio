...
const screenshotStorage = multer.diskStorage({
  destination (req, file, callback) {
    callback(null, path.join(utils.getMediaUploadPath(), 'screenshots'));
  },
...
const avatarStorage = multer.diskStorage({
  destination (req, file, callback) {
    callback(null, path.join(utils.getMediaUploadPath(), 'avatars'));
  },
  filename (req, file, callback) {
    callback(null, `${Date.now()}_${String(file.originalname).toLowerCase().replace(/\s/g, '_').trim()}`);
  },
...
const recordingStorage = multer.diskStorage({
  destination (req, file, callback) {
    callback(null, path.join(utils.getMediaUploadPath(), 'recordings'));
  },
  filename (req, file, callback) {
    callback(null, `${Date.now()}_${String(file.originalname).toLowerCase().replace(/\s/g, '_').trim()}`);
  },
